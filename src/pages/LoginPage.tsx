    ...
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'var(--bg)', padding:20 }}>
      <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:18, padding:'36px 32px', width:'100%', maxWidth:380 }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {error && (
            <div style={{ marginBottom:12, padding:'8px 12px', background: error.includes('correo') ? 'var(--green-bg)' : 'var(--red-bg)', border:`1px solid ${error.includes('correo') ? 'var(--green)' : 'var(--red)'}`, borderRadius:8, fontSize:12, color: error.includes('correo') ? 'var(--green)' : 'var(--red)' }}>
              {error}
            </div>
          )}
